...
class MaxEsenNICModel(caption_solver):
	def __init__(self, arguments):
		super(MaxEsenNICModel, self).__init__(arguments)
		self.lstm_step = MaxEsenNICModelFunction(arguments)

class MaxEsenNICModelFunction(torch.nn.Module):
	def __init__(self, arguments):
		super(MaxEsenNICModelFunction, self).__init__()
...
def train(arguments):
	...
	maxesen = MaxEsenNICModel(arguments)
	maxesen.cuda()
	maxesen.train()

	build_model = Model()

	grad_compute = torch.optim.Adam(maxesen.parameters(), lr=5e-4, weight_decay=0)
	...
	while True:
		if flag:
			...
			if epoch == 0:
				...
				maxesen.rand_dropout_p = arguments.rand_dropout_p
		...
		cost = build_model(maxesen(global_feature, local_feature, captions), captions[:,1:], musk[:,1:])
		cost.backward()
		...
		if (update % arguments.validFreq == 0):
			loss, result, scores = metrics.evaluate(maxesen, build_model, homogeneousdata, vars(arguments))
			...
			if score > max_score:
				max_score = score
				torch.save(maxesen.state_dict(), 'model-final.pth')
				print("Saved final model")
	
			torch.save(maxesen.state_dict(), 'model.pth')
			print("Saved model")
